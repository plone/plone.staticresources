...
module.exports = function(grunt) {{
    ...
    var lessPaths = {lesspaths},
        ...
        modifyVarsNoEmbed = {modifyvarsnoembed};
    grunt.initConfig({{
                ...
                try:
                    if callable(resource):
                        ...
                        try:
                            ...
                            pass
                        full_file_name = (
                            temp_resource_folder + "/" + file_name + file_type
                        )
...
modify_vars["staticPath"] = (
    "'" + os.path.join(os.path.dirname(plone.staticresources.__file__), "static") + "'"
)
if HAS_BARCELONETA:
    modify_vars["barcelonetaPath"] = (
        "'"
        + os.path.join(os.path.dirname(plonetheme.barceloneta.__file__), "theme")
        + "'"
    )

less_vars_params = {"site_url": "LOCAL"}
...
for name, value in lessvariables.items():
    ...
    if "LOCAL" in t:
        t_object = portal.unrestrictedTraverse(
            str(t.replace("LOCAL/", "").replace('\\"', "")), None
        )
        if t_object:
            ...
            print(
                "No file found: " + str(t.replace("LOCAL/", "").replace('\\"', ""))
            )  #
    else:
        modify_vars[name] = t

# remove icon-font-path to get value from less file.
modify_vars_no_embed = modify_vars.copy()
del modify_vars_no_embed['icon-font-path']
...
for bkey, bundle in bundles.items():
    ...
    if compile_path:
        ...
        if bundle.csscompilation:
            ...
            css_target_path = resource_to_dir(
                portal.unrestrictedTraverse("/".join(css_compilation[:-1]))
            )
        ...
        if bundle.jscompilation:
            ...
            js_target_path = resource_to_dir(
                portal.unrestrictedTraverse("/".join(js_compilation[:-1]))
            )
    ...
    if bundle.compile:
        ...
        for resource in bundle.resources:
            ...
            if res_obj.css:
                ...
                for css_file in res_obj.css:
                    ...
                    dest_path = "{}/{}".format(css_target_path, css_target_name)
        ...
        bundle_grunt_tasks += COMPILE_TASK_TEMPLATE.format(
            name=bkey, tasks=json.dumps(tasks)
        )
...
with open("Gruntfile.js", "w") as gruntfile:
    gruntfile.write(
        GRUNTFILE_TEMPLATE.format(
            ...
            modifyvars=json.dumps(modify_vars, indent=12, sort_keys=True).replace(
                "\\\\\\", "\\"
            ),  # replace 3 backslash by one.
            modifyvarsnoembed=json.dumps(modify_vars_no_embed, indent=12, sort_keys=True).replace(
                "\\\\\\", "\\"
            ),  # replace 3 backslash by one.
